...
-- because of this. STRUCTURALLY ISOLATED exactly as the rest of SECTION 10:
-- discovery_candidate gains NO column (the lineage FK below points TO it,
-- never the reverse); crawl_trigger is untouched. Mirrored by
-- db/migrations/0010_add_freshness_layer.sql.
-- -----------------------------------------------------------------------------

-- Stored ONLY as freshness_observation.execution_mode_at_check, an immutable
-- observation-time snapshot — never a mutable column on freshness_target
-- (effective mode is always computed fresh at runtime; persisting a verdict
-- on the target would let it go stale the moment an unrelated
-- discovery_source review expires).
...
-- A DEDICATED enum, deliberately not an addition to crawl_trigger, which
-- keeps its single ratified value ('MANUAL') untouched. Reusing crawl_trigger
-- would put a freshness-triggered value on the same enum/table the
-- radar/discovery ingest path (crawl_run, fetched_page, adapters.ingest())
-- uses, making it easier for a future shortcut to route a freshness check
-- through candidate-discovery code. A dedicated enum on an unrelated table
-- cannot do that.
CREATE TYPE freshness_trigger AS ENUM ('MANUAL', 'SCHEDULED_FRESHNESS');

CREATE TABLE freshness_target (
    id                      UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    robot_id                UUID NOT NULL REFERENCES robot(id) ON DELETE CASCADE,
    discovery_source_id     UUID NOT NULL REFERENCES discovery_source(id) ON DELETE RESTRICT,
    url                     TEXT NOT NULL,
...
COMMENT ON TABLE freshness_observation IS
    ...
    'Append-only by convention. discovery_candidate_id is the explicit observation->work '
    'lineage; NULL except on a CHANGED/CHANGE_FOUND result that created or reused work.';

CREATE INDEX idx_freshness_target_due ON freshness_target (active, last_checked_at);
CREATE INDEX idx_freshness_observation_target
    ON freshness_observation (freshness_target_id, checked_at DESC);
CREATE INDEX idx_freshness_observation_candidate
