...
class Reader:
    ...
    def get(self, url, *, as_json=True):
        ...
        try:
            with self.opener.open(Request(url, headers=headers, method="GET"), timeout=min(10, remaining)) as response:
                if response.status != 200:
                    raise ObservationError("PENDING", f"Expected HTTP 200, received {response.status}")
...
def origin(value):
    ...
    if (p.scheme != "https" or not p.hostname or p.username or p.password
            or p.port not in (None, 443) or p.query or p.fragment or p.path not in ("", "/")):
        raise ObservationError("UNVERIFIED", "Configure an HTTPS origin without path or credentials")
...
def ci_observation(reader, run, expected_sha):
    ...
    if (run.get("head_sha") != expected_sha or run.get("path") != ".github/workflows/ci.yml"
            or run.get("repository", {}).get("full_name") != REPO):
        return row("CI", "UNVERIFIED", "Run does not match repository, workflow and exact commit", url)
    if run.get("status") != "completed":
        return row("CI", "PENDING", "CI is still running", url)
    jobs = pages(reader, f"{GH}/actions/runs/{run['id']}/attempts/{run['run_attempt']}/jobs", "jobs")
    failures = []
    for job in jobs:
        if job.get("conclusion") not in ("success", "skipped"):
            failed_steps = [str(s.get("name", "unnamed")) for s in job.get("steps", [])
                            if s.get("conclusion") in ("failure", "cancelled", "timed_out")]
            failures.append(f"{job.get('name', 'job')}: {', '.join(failed_steps) or job.get('conclusion') or 'pending'}")
    if run.get("conclusion") != "success" or failures:
        return row("CI", "FAIL", f"Attempt {run['run_attempt']}: {run.get('conclusion')}; " + "; ".join(failures), url)
    if not jobs:
        return row("CI", "UNVERIFIED", "Completed run has no job evidence", url)
    return row("CI", "PASS", f"Attempt {run['run_attempt']}: {len(jobs)} jobs; includes post-job cleanup", url)


def current_main(reader, sha):
    current = reader.get(f"{GH}/git/ref/heads/main")["object"]["sha"]
    if current != sha:
        return row("main", "SUPERSEDED", f"Current main is {current}; historical commit is not certified live")
...
def netlify_observation(reader, env, sha):
    ...
    if not site_id or not env.get("WATCHER_NETLIFY_TOKEN"):
        return row(name, "UNVERIFIED", "Configure WATCHER_NETLIFY_SITE_ID and WATCHER_NETLIFY_TOKEN")
    ...
    if site.get("id") != site_id or origin(site.get("ssl_url", "")) != web:
        return row(name, "UNVERIFIED", "Site ID or primary production origin does not match configuration")
    deploy = site.get("published_deploy") or {}
    actual = deploy.get("commit_ref")
    evidence = f"https://app.netlify.com/sites/{quote(site.get('name', ''), safe='')}/deploys/{quote(deploy.get('id', ''), safe='')}"
    if not actual:
    ...
    return row(name, "PASS", f"Published deployment {deploy['id']} serves exact commit {sha}", evidence)
...
def vercel_observation(reader, env, sha, role):
    ...
    if not project or not alias or not env.get("WATCHER_VERCEL_TOKEN"):
        return row(name, "UNVERIFIED", f"Configure {prefix}PROJECT_ID, {prefix}ALIAS and WATCHER_VERCEL_TOKEN")
    ...
    alias_data = reader.get(f"https://api.vercel.com/v4/aliases/{quote(alias, safe='')}?{urlencode(query)}")
    if alias_data.get("projectId") != project or alias_data.get("alias") != alias or alias_data.get("redirect"):
    ...
    data = reader.get(f"https://api.vercel.com/v13/deployments/{quote(deployment_id, safe='')}?{urlencode(query)}")
    ...
    return row(name, "PASS", f"Alias serves deployment {deployment_id} at {sha}", "https://" + alias)
...
def health_observations(reader, env):
    ...
    web = origin(env.get("WATCHER_WEB_ORIGIN", "https://humanoidonline.com"))
    checks = []
    for path, expected in (("/health", {"status": "ok"}), ("/ready", {"status": "ok", "database": "up"})):
        url = api + path
        def api_check(url=url, expected=expected):
            data = reader.get(url)
            ok = isinstance(data, dict) and all(data.get(k) == v for k, v in expected.items())
            return row(url, "PASS" if ok else "PENDING", "Expected health payload" if ok else "Unexpected health payload", url)
    ...
    for path in ("/", "/robots", "/manufacturers", "/find-a-humanoid"):
        ...
        def web_check(url=url):
            ...
            return row(url, "PASS" if ok else "PENDING", "HTTP 200 and HTML marker" if ok else "Unexpected page content", url)
...
def snapshot(reader, env, run_id, sha):
    ...
    if not production:
        ...
        checks.append(row("Production", "NOT_CHECKED", "Production observation only follows a main push CI run"))
        return scope, checks
    ...
    for role in ("API", "WEB"):
        checks.append(observe(f"Vercel {role.lower()} production", lambda role=role: vercel_observation(reader, env, sha, role)))
    if all(c["state"] == "PASS" for c in checks):
        ...
        checks.append(observe("Netlify production recheck", lambda: netlify_observation(reader, env, sha)))
        for role in ("API", "WEB"):
            checks.append(observe(f"Vercel {role} recheck", lambda role=role: vercel_observation(reader, env, sha, role)))
        checks.append(observe("main recheck", lambda: current_main(reader, sha)))
    else:
        checks.append(row("Live health", "NOT_CHECKED", "Waiting for CI and all production identities to pass"))
    return scope, checks
...
def markdown(report):
    ...
    lines = [f"# HumanoidOnline watcher: {report['result']}", "",
    ...
    lines += ["", "Read-only observation; no repairs, reruns, merges, deployments or data writes.",
              "PASS is limited to the stated scope and observation time. HTTP probes do not replace browser journey tests.", ""]
    return "\n".join(lines)


def watch(reader, env, run_id, sha, deadline, interval, output, *, clock=time.monotonic, sleep=time.sleep):
    while True:
        try:
            ...
            scope, checks = "unverified", [row("Observation", "UNVERIFIED", "Unexpected provider response shape")]
        result = outcome(checks, expired=clock() >= deadline)
        report = {"version": 1, "repository": REPO, "sha": sha, "run_id": run_id, "scope": scope,
                  "observed_at": datetime.now(timezone.utc).isoformat(), "result": result, "checks": checks}
